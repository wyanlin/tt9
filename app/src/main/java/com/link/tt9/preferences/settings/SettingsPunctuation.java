...
import com.link.tt9.ui.tray.SuggestionsBar;
...
class SettingsPunctuation extends SettingsInput {
	...
	// add default chars for 0 and 1 2025/10/11
...
//		}
	...
	public ArrayList<String> getChars1AsList(@Nullable Language language) {
		...
		if (safeChars == null) {
			Logger.w(TAG, "getChars1AsList: no chars for language " + language.getId());
			safeChars = SettingsStatic.CHAR_1;
		}

		return getCharsAsList(language, safeChars, language.getKeyCharacters(1));
	}
	...
	public ArrayList<String> getChars0AsList(@Nullable Language language) {
		...
		if (safeChars != null) {
			...
			Logger.w(TAG, "getChars0AsList: no chars for language " + language.getId());
			safeChars = SettingsStatic.CHAR_0;
		}
